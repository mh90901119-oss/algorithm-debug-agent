...
/** Collector 返回给 Core 的 manifest 与不可变产物路径。 */
public record MethodPathCollectionResult(
    ...
    public MethodPathCollectionResult {
        ...
        rawTrace = inside(root, rawTrace, "rawTrace");
        filteredTrace = inside(root, filteredTrace, "filteredTrace");
        stdoutLog = inside(root, stdoutLog, "stdoutLog");
        stderrLog = inside(root, stderrLog, "stderrLog");
        manifest = Objects.requireNonNull(manifest, "manifest");
        if (!manifest.caseId().equals(request.caseId())
                || !manifest.contextId().equals(request.contextId())
    ...
    private static Path inside(Path root, Path value, String name) {
        ...
        if (!checked.startsWith(root) || checked.equals(root)) {
            throw new IllegalArgumentException(name + " 越过 Collection 目录");
        }
        return checked;
    }
}
